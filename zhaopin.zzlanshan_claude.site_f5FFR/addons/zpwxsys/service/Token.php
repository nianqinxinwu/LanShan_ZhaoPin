<?php
    ...
    use addons\zpwxsys\lib\exception\ForbiddenException;
...
//        $identities = cache($token);
        ...
        public static function getCurrentCid($token)
        {
            $companyid = self::getCurrentCompanyTokenVar('companyid',$token);
            $cscope = self::getCurrentCompanyTokenVar('cscope',$token);
            if ($cscope == ScopeEnum::Super) {
                // 只有Super权限才可以自己传入companyid
                // 且必须在get参数中，post不接受任何companyid字段
                $userID = input('get.companyid');
                if (!$userID) {
                    throw new ParameterException(['msg' => '没有指定需要操作的用户对象']);
                }
                return $userID;
