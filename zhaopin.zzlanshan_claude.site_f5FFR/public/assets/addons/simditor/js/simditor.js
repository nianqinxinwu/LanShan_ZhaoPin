  ...
  Clipboard.prototype._getPasteContent = function(callback) {
    ...
    return setTimeout((function(_this) {
      return function() {
        ...
        _this.editor.body.get(0).innerHTML = DOMPurify && _this.opts.dompurify.enabled ? DOMPurify.sanitize(state.html, _this.opts.dompurify.options) : state.html;
...
Simditor = (function(superClass) {
  ...
  Simditor.prototype.setValue = function(val) {
    ...
    this.body.get(0).innerHTML = DOMPurify && this.opts.dompurify.enabled ? DOMPurify.sanitize(val, this.opts.dompurify.options) : val;
  ...
  Simditor.prototype.getValue = function() {
    ...
    Simditor.prototype.insertHTML = function(html, options) {
        ...
        html = DOMPurify && _this.opts.dompurify.enabled ? DOMPurify.sanitize(html, _this.opts.dompurify.options) : html;
        ...
        if (!options.skipClean) {
            this.formatter.format($temp);
            this.formatter.decorate($temp);
        }
        ...
        // 返回插入的节点
        return $(firstNode).parent().find(firstNode).nextUntil($(lastNode).next());
    };
...
ImagePopover = (function(superClass) {
  ...
  ImagePopover.prototype._initUploader = function() {
    ...
    ImagePopover.prototype._resizeImg = function(inputEl, onlySetVal) {
        ...
        var isPercentage = inputValue && inputValue.toString().indexOf('%') !== -1;
        ...
        if (inputEl.is(this.widthEl)) {
            ...
            height = value;
            if(height.toString().indexOf('%')===-1 && height!=='') {
                width = this.width * value / this.height;
                this.widthEl.val(width);
            }
            this.target.attr('height', height);
        }
        if (!onlySetVal) {
            return this.editor.trigger('valuechanged');
        }
  ...
  ImagePopover.prototype._restoreImg = function() {
      var ref, size, originalWidth, originalHeight;

        // 重新读取原图宽高
      if (this.target[0].naturalWidth && this.target[0].naturalHeight) {
          originalWidth = this.target[0].naturalWidth;
          originalHeight = this.target[0].naturalHeight;
      } else if (this.target[0].width && this.target[0].height) {
          originalWidth = this.target[0].width;
          originalHeight = this.target[0].height;
      } else {
          // 如果无法直接获取，创建临时图片对象获取
          var tempImg = new Image();
          tempImg.src = this.target.attr('src');
          originalWidth = tempImg.naturalWidth || tempImg.width || this.width;
          originalHeight = tempImg.naturalHeight || tempImg.height || this.height;
      }

      size = ((ref = this.target.data('image-size')) != null ? ref.split(",") : void 0) || [originalWidth, originalHeight];
  ...
  ImagePopover.prototype.show = function() {
    ...
    this.alt = $img.attr('alt');
      if ($img.hasClass('uploading')) {
          ...
          if ($img.attr('width')) {
              this.widthEl.val($img.attr('width'));
          } else {
              // this.widthEl.val(this.width);
          }
          if ($img.attr('height')) {
              this.heightEl.val($img.attr('height'));
          } else {
